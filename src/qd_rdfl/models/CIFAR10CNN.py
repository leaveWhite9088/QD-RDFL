...
from src.qd_rdfl.datasets.CIFAR10Dataset import CIFAR10Dataset
from src.qd_rdfl.utils.UtilsCIFAR10 import UtilsCIFAR10
from src.qd_rdfl.global_variable import global_cifar10_parent_path
...
class CIFAR10CNN(nn.Module):
    ...
    def train_model(self, train_loader, criterion, optimizer, num_epochs=20, device='cpu',
        ...
        for epoch in range(num_epochs):
            running_loss = 0.0
            UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"Epoch {epoch + 1}/{num_epochs} started...")  # 打印每个epoch的开始
            ...
            # 每个 epoch 结束时输出平均损失
            UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"Epoch {epoch + 1} completed. Average Loss: {running_loss / len(train_loader):.4f}")
    ...
    def evaluate(self, test_loader, device='cpu'):
        ...
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"Loss: {avg_loss:.4f}, Accuracy: {accuracy:.2f}%")
    ...
    def save_model(self, file_path):
        ...
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"Model saved to {file_path}")
    ...
    def load_model(self, file_path):
        ...
        self.eval()
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"Model loaded from {file_path}")

# 使用CIFAR10数据集，训练cnn
def train_cifar10_cnn(data_dir, model_save_path="../data/model/cifar10_cnn_model"):
    """
    使用CIFAR10数据集，训练cnn
    ...
    # 创建训练和测试 DataLoader
    train_loader = UtilsCIFAR10.create_data_loader(train_data, train_labels, batch_size=128, shuffle=True)
    test_loader = UtilsCIFAR10.create_data_loader(test_data, test_labels, batch_size=128, shuffle=False)
...
def fine_tune_model(model, train_loader, test_loader, num_epochs=5, device='cpu', lr=1e-5, model_path=None):
    ...
    :param model: 已训练的CNN模型 (CIFAR10CNN)
    ...
    if model_path and os.path.exists(model_path):
        ...
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"加载模型来自 {model_path}")
    else:
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "未提供或未找到模型路径，跳过加载。")

    # 评估原模型
    UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "原模型评估：")
    ...
    for epoch in range(num_epochs):
        ...
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"Epoch {epoch + 1}/{num_epochs}, Loss: {avg_loss:.4f}")
    ...
    # 如果效果比原模型好，就更新原模型
    UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "新模型评估：")
...
def fine_tune_model_without_replace(model, train_loader, test_loader, num_epochs=20, device='cpu', lr=1e-5,
    ...
    if model_path and os.path.exists(model_path):
        model.load_model(model_path)  # 加载先前保存的模型
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"加载预训练模型来自 {model_path}")
    else:
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "未提供或未找到预训练模型路径，跳过加载。")
    ...
    if first_epoch_loss is not None and last_epoch_loss is not None:
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "loss 差为：")
        ...
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"单位数据 loss 差为：{unitDataLossDiff:.6f}")
    else:
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "未能计算 loss 差。")
