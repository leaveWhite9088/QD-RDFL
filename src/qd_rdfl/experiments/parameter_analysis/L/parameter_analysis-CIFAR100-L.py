...
def train_model_with_cpc(matching, cpcs, test_images, test_labels, literation, avg_f_list, adjustment_literation):
    ...
    for item in matching.items():
        ...
        tempmodel, accuracy = fine_tune_model(model, train_loader, test_loader, num_epochs=5, device=device,
                                                  lr=1e-5,
                                                  model_path=f"{project_root}/data/model/cifar100_cnn_model")

    return UtilsCIFAR100.normalize_list(avg_f_list), accuracy
...
if __name__ == "__main__":
    ...
    for adjustment_literation in range(2, 21):
        ...
        UtilsCIFAR100.print_and_log(global_cifar100_parent_path, f"accuracy_list: {accuracy_list}")

    UtilsCIFAR100.print_and_log(global_cifar100_parent_path, "最终的 adjustment_literation_list 列表：")
    UtilsCIFAR100.print_and_log(global_cifar100_parent_path, f"adjustment_literation_list: {adjustment_literation_list}")
