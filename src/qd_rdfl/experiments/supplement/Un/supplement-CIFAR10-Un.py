...
def evaluate_data_quality(dataowners, report_rate):
    """
    评价DataOwner的数据质量
    :param dataowners: DataOwner对象列表
    :return: 归一化后的数据质量列表
    ...
    for i, do in enumerate(dataowners):
        ...
        if i == 0:
            UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"准备调整avg_mse：{avg_mse}")
            avg_mse = avg_mse * (1 - report_rate)
            UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"avg_mse调整完毕：{avg_mse}")
...
if __name__ == "__main__":
    ...
    for n in [9]:
        ...
        while True:
            ...
            Un_list = []
            for rate in [-0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6]:

                UtilsCIFAR10.print_and_log(global_cifar10_parent_path,
                                         f"----- literation {literation + 1}: 计算 DataOwner 的数据质量 -----")
                # -0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6
                report_rate = rate
                avg_f_list = evaluate_data_quality(dataowners, report_rate)
                UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "DONE")
                ...
                # TODO avg_f_list, last_xn_list需要清空
                ...
                if literation == adjustment_literation + 1:
                    # 在这里算0号客户端的效用，然后添加到列表
                    new_qn_list = [a * b for a, b in zip(xn_list, avg_f_list)]

                    # 计算被观察的DO的效用
                    new_U_qn = new_qn_list[0] * best_Eta / sum(new_qn_list) - xn_list[0] * Lambda * Rho
                    Un_list.append(new_U_qn)
        ...
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path, Un_list)