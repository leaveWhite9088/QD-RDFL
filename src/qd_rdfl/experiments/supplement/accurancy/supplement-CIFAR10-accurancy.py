...
from src.qd_rdfl.models.CIFAR10CNN import CIFAR10CNN, fine_tune_model, fine_tune_model_without_replace
...
from src.qd_rdfl.global_variable import global_cifar10_parent_path,Lambda,Rho,Alpha,Epsilon,adjustment_literation
...
def ready_for_task(Lambda, Rho, Alpha, Epsilon, N, M, SigmaM, data_dir):
    ...
    # 切分数据
    UtilsCIFAR10.split_data_to_dataowners(dataowners, train_data, train_labels)
...
def init_model():
    ...
    train_loader = UtilsCIFAR10.create_data_loader(selected_train_data, selected_train_labels, batch_size=128,
                                                   shuffle=True)
    test_loader = UtilsCIFAR10.create_data_loader(test_data, test_labels, batch_size=64, shuffle=False)
...
# 给数据集添加噪声
def dataowner_add_noise(dataowners, rate, report_rate, client_rate):
    """
    给数据集添加噪声
    :param dataowners:
    ...
    for i, do in enumerate(dataowners):
        if i < client_rate * N:
            random_num = 0.5 * rate * (1 + report_rate)
        else:
            random_num = 0.5 * rate
        UtilsCIFAR10.add_noise(do, severity=random_num)
...
def evaluate_data_quality(dataowners):
    ...
    UtilsCIFAR10.print_and_log(global_cifar10_parent_path,
                             f"归一化后的数据质量列表avg_f_list: {UtilsCIFAR10.normalize_list(avg_f_list)}")
...
if __name__ == "__main__":
    ...
    for n in [9]:
        ...
        accuracy_dict[0.0] = []
        accuracy_dict[0.5] = []
        for ite_report_rate in [-0.5, 0.0, 0.5]:
            UtilsCIFAR10.print_and_log(global_cifar10_parent_path,
                                       "---------------------------------- 准备工作 ----------------------------------")
            ...
            data_dir = f"{project_root}/data/dataset/CIFAR10"  # CIFAR10批处理文件所在目录
            dataowners, modelowner, cpcs, test_data, test_labels = ready_for_task(Lambda, Rho, Alpha, Epsilon, N, M,
                                                                                  SigmaM,
                                                                                  data_dir)
            UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "DONE")

            if dataowners_backup == []:
                dataowners_backup = dataowners.copy()
            else:
                dataowners = dataowners_backup.copy()
            ...
            while True:
                ...
                if literation == 0:
                    ...
                    dataowner_add_noise(dataowners, 0.1, report_rate=ite_report_rate, client_rate=client_rate)
                    UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "DONE")
                ...
                accuracy_dict[ite_report_rate].append(new_accuracy)
                UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "DONE")
        ...
        UtilsCIFAR10.print_and_log(global_cifar10_parent_path,
                                 f"accuracy_dict[{ite_report_rate}]: {accuracy_dict[ite_report_rate]}")

    UtilsCIFAR10.print_and_log(global_cifar10_parent_path, "最终的字典：")
    UtilsCIFAR10.print_and_log(global_cifar10_parent_path, f"accuracy_dict: {accuracy_dict}")
