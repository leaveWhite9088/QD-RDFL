...
def init_model(rate):
    ...
    if os.path.exists(model_save_path) and os.path.exists(vocab_path):
        ...
        # 创建数据加载器
        train_loader = UtilsIMDB.create_data_loader(selected_train_texts, selected_train_labels, vocab, max_length=200, batch_size=64, shuffle=True)
        test_loader = UtilsIMDB.create_data_loader(test_texts, test_labels, vocab, max_length=200, batch_size=64, shuffle=False)
...
def dataowner_add_noise(dataowners, rate):
    ...
    for i, do in enumerate(dataowners):
        ...
        UtilsIMDB.print_and_log(global_imdb_parent_path, f"DataOwner{i + 1}: noise random: {random_num}, type: {noise_type}")
