...
def init_model():
    ...
    :return: 初始化后的模型
    """
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, "初始化模型中...")

    # 假设初始比例为1% (rate=0.01)
    rate = 0.01
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"初始数据占CIFAR10的比例：{rate * 100}%")
    ...
    if os.path.exists(model_save_path):
        UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"{model_save_path} 存在，加载初始化模型")
    ...
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, "初始化模型的准确率：")
...
def dataowner_add_noise(dataowners, rate):
    ...
    for i, do in enumerate(dataowners):
        ...
        UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"DataOwner{i + 1}: noise random: {random_num}")
...
def evaluate_data_quality(dataowners):
    ...
    for i, do in enumerate(dataowners):
        ...
        for j, (mse, snr) in enumerate(zip(mse_scores, snr_scores)):
            # 如果需要详细日志，可以取消下面注释
            # UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"DataOwner{i + 1}: Image {j + 1}: MSE = {mse:.4f}, SNR = {snr:.2f} dB")
            mse_sum += mse
        avg_mse = mse_sum / len(mse_scores)
        avg_f_list.append(1 - avg_mse)

    UtilsCIFAR10.print_and_log(global_cifar_parent_path, "DataOwners自行评估数据质量：")
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"数据质量列表avg_f_list: {avg_f_list}")
...
def calculate_optimal_payment_and_data(avg_f_list, last_xn_list):
    ...
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, "Stackelberg均衡结果：")
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"ModelOwner的最优Eta = {eta_opt:.4f}")
    xn_list = []
    for i, xi in enumerate(x_opt):
        UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"DataOwner{i + 1}的最优x_{i + 1} = {xi:.4f}")
        xn_list.append(xi)
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"每个DataOwner应该贡献数据比例 xn_list = {xn_list}")
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"ModelOwner的最大效用 U(Eta) = {U_opt:.4f}")
...
def compute_contribution_rates(xn_list, avg_f_list, best_Eta):
    ...
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"ModelOwner的最优总支付：{best_Eta:.4f}")
...
def match_data_owners_to_cpc(xn_list, cpcs):
    ...
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"Matching结果: {matching}")
    return matching
...
def train_model_with_cpc(matching, cpcs, test_images, test_labels, literation, avg_f_list, adjustment_literation, N):
    ...
    if literation == adjustment_literation:
        UtilsCIFAR10.print_and_log(global_cifar_parent_path, "重新调整fn，进而调整xn、Eta")
        avg_f_list = [0] * N
        for item in matching.items():
            ...
            if cpc_data_len == 0:
                UtilsCIFAR10.print_and_log(global_cifar_parent_path, "数据量为0，跳过此轮评估")
                continue
        ...
        UtilsCIFAR10.print_and_log(global_cifar_parent_path, "经过服务器调节后的真实数据质量：")
        UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"数据质量列表avg_f_list: {avg_f_list}")
        normalized_avg_f_list = UtilsCIFAR10.normalize_list(avg_f_list)
        UtilsCIFAR10.print_and_log(global_cifar_parent_path,
                                   f"归一化后的数据质量列表avg_f_list:{normalized_avg_f_list}")
    ...
    for item in matching.items():
        ...
        if cpc_data_len == 0:
            UtilsCIFAR10.print_and_log(global_cifar_parent_path, "数据量为0，跳过此轮调整")
            continue
...
if __name__ == "__main__":
    ...
    for n in [9, 19, 29, 39, 49, 59, 69, 79, 89, 99]:
        ...
        UtilsCIFAR10.print_and_log(global_cifar_parent_path, "DONE")
        ...
        while True:
            ...
            UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"----- literation {literation + 1}: 模型训练 -----")
            ...
            literation += 1
            if literation > adjustment_literation + 1:
                UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"U_Eta_list: {U_Eta_list}")
                UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"U_qn_list: {U_qn_list}")
                break

    UtilsCIFAR10.print_and_log(global_cifar_parent_path, "最终的列表：")
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"U_Eta_list: {U_Eta_list}")
    UtilsCIFAR10.print_and_log(global_cifar_parent_path, f"U_qn_list: {U_qn_list}")
