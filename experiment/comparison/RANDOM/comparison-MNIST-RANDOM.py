...
if __name__ == "__main__":
    UtilsMNIST.print_and_log(global_minst_parent_path,
                             f"**** {global_minst_parent_path}-random 运行时间： {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ****")
    ...
    for n in range(1, 101):
        ...
        for i, xi in enumerate(random_x_opt):
            UtilsMNIST.print_and_log(global_minst_parent_path, f"random: DataOwner{i + 1}的最优x_{i + 1} = {xi:.4f}")
        ...
        UtilsMNIST.print_and_log(global_minst_parent_path, "本轮的列表：")
